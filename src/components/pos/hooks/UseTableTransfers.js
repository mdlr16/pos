// Sistema de transferencias de mesa
const TableTransferSystem = {
  ...
  TableTransferModal: ({ isOpen, onClose, currentMesa, allMesas, onTransfer }) => {
                ...
                {currentMesa.productos?.map(product => (
                  <div key={product.id} className="flex items-center justify-between py-2 border-b last:border-b-0">
                    ...
                    <div className="flex items-center space-x-2">
                      <input
                        type="checkbox"
                        checked={transferItems.includes(product.id)}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setTransferItems([...transferItems, product.id]);
                          } else {
                            setTransferItems(transferItems.filter(id => id !== product.id));
                          }
                        }}
                        className="w-4 h-4"
                      />
    ...
    const handleConfirmTransfer = async () => {
      ...
      try {
        ...
        alert('Error realizando la transferencia');
      }
    };
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="p-6 border-b bg-gray-50">
            <h2 className="text-xl font-bold mb-4">Transferencia de Mesa</h2>
            <div className="flex space-x-2">
              {Object.entries(this.operations).map(([key, value]) => (
                <button
                  key={key}
...
// Integración en TableModal - Agregar botón de transferencia
const TableModalWithTransfer = ({ mesa, ...props }) => {
  const [showTransferModal, setShowTransferModal] = useState(false);

  // En el footer del TableModal, agregar:
  ...
  const handleTransfer = async (transferData) => {
    ...
    console.log('Transferencia a realizar:', transferData);
    
    // Llamar a API para transferir
  ...
  return (
    <>
      {/* TableModal original con botón de transferencia */}
      <TableModal mesa={mesa} {...props} extraButtons={[transferButton]} />
      
      {/* Modal de transferencia */}
      {showTransferModal && (
        <TableTransferSystem.TableTransferModal
          isOpen={showTransferModal}
          onClose={() => setShowTransferModal(false)}
          currentMesa={mesa}
          allMesas={props.allMesas || []}
