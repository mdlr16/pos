...
const handleOpenSalesHistoryModal = () => {
  console.log("Opening sales history modal");
...
const handleOpenSalesHistoryModal = async () => {
  ...
  try {
    const cotizacionesResponse = await fetch(`${variables.SPOS_URL}/custom/pos/frontend/ajax_pos_siel.php?action=getCotizaciones`);
    const pedidosResponse = await fetch(`${variables.SPOS_URL}/custom/pos/frontend/ajax_pos_siel.php?action=getPedidos`);
    const facturasResponse = await fetch(`${variables.SPOS_URL}/custom/pos/frontend/ajax_pos_siel.php?action=getFacturas`);
      ...
      if (selectedMethod) {
        ...
        const newPayments = [
          ...
          { 
            ...
            idBank: selectedMethod.rowid  // ID del método de pago
          }
        ];
        setPayments(newPayments);
        setSaldo(saldo - newPaymentAmount);
  ...
  const handleSaveFactura = async () => {
    try {
      ...
      payments.forEach((payment, index) => {
        ...
        paymentData[`idbank${index + 1}`] = payment.idBank; // Usar la propiedad idBank que ya está almacenada
        paymentData[`tipopago${index + 1}`] = payment.method; // Usar el label como tipo de pago
      });
      ...
      const data = {
        ...
        warehouse: terminal.fk_warehouse,
        clase: tipoVenta === "Cotizacion" ? 0 : tipoVenta === "Pedido" ? 2 : 3,
        type: 0,//tipoVenta === "Pedido" ? 2 : tipoVenta === "Factura" ? 3 : ,
        old_type: oldType, // Asignar el tipo con el que nació el ticket
        discount_percent: customerDiscount.toString(),
        discount_qty: 0,
        lines: cart.map(item => ({
          id: item.idLine || 0,
        ...
        id_place: 0,
        note: generalNotes,
        mode: 2, // Este modo es el que acepta el pos.class para crear la factura
        points: 0,
        idCoupon: 0,
        ret_points: 0,
        ...paymentData, // Añadir los pagos dinámicamente
        id_vendor: selectedCategory,
        is_fel: tipoVenta === "Factura" && isFel ? 1 : 0,
        cashId: terminal.rowid,
        extraFields: extraFields // Añadir los campos extras
      ...
      // Hacer la solicitud POST para guardar el ticket con los pagos
      const response = await fetch(`${variables.SPOS_URL}/custom/pos/frontend/ajax_pos_siel.php?action=saveTickets`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      const result = await response.json();
      if (result.error && result.error.value === 0) {
        setAlert({ show: true, type: 'success', message: 'Ticket guardado correctamente.' });
        ...
        setExtraFields({});
        setPayments([]);
        setIsPaymentModalOpen(false); // Cerrar el modal de pagos
      } else {
 ...
 {/* Modal para Cotizaciones, Pedidos y Facturas */}
  ...
  <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
    <div className="bg-white p-6 rounded-lg shadow-lg w-11/12 md:w-2/3 max-h-full overflow-y-auto">
      ...
      {/* Pestañas para Cotizaciones, Pedidos y Facturas */}
      <div className="tabs">
        <button 
          className={`tab ${activeTab === "Cotizaciones" ? "tab-active" : ""}`}
        ...
        {activeTab === "Cotizaciones" && (
          <div className="space-y-2">
            {salesData.cotizaciones.length > 0 ? (
              salesData.cotizaciones.map((item, index) => (
                <div key={index} className="border p-4 rounded-md">
                  <h3 className="font-bold">Cotización #{item.id}</h3>
                  <p>Cliente: {item.cliente}</p>
                  <p>Total: Q{parseFloat(item.total).toFixed(2)}</p>
                  <button className="btn btn-primary btn-sm mt-2" onClick={() => handleEditTicket(item)}>
                    Seleccionar
                  </button>
                </div>
              ))
        ...
        {activeTab === "Pedidos" && (
          <div className="space-y-2">
            {salesData.pedidos.length > 0 ? (
              salesData.pedidos.map((item, index) => (
                <div key={index} className="border p-4 rounded-md">
                  <h3 className="font-bold">Pedido #{item.id}</h3>
                  <p>Cliente: {item.cliente}</p>
                  <p>Total: Q{parseFloat(item.total).toFixed(2)}</p>
                  <button className="btn btn-primary btn-sm mt-2" onClick={() => handleEditTicket(item)}>
                    Seleccionar
        ...
        {activeTab === "Facturas" && (
          <div className="space-y-2">
            {salesData.facturas.length > 0 ? (
              salesData.facturas.map((item, index) => (
                <div key={index} className="border p-4 rounded-md">
                  <h3 className="font-bold">Factura #{item.id}</h3>
                  <p>Cliente: {item.cliente}</p>
                  <p>Total: Q{parseFloat(item.total).toFixed(2)}</p>
                  <button className="btn btn-primary btn-sm mt-2" onClick={() => handleEditTicket(item)}>
                    Seleccionar
      ...
      <div className="flex justify-end mt-4">
        <button className="btn btn-secondary" onClick={() => setIsSalesHistoryModalOpen(false)}>
          Cerrar
        </button>
      </div>
    </div>
